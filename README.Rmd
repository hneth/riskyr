...
```{r ex1_prism, eval = FALSE, fig.width = 7.2, fig.height = 5.5, fig.show = 'hold'}
...
1. by condition (`by = "cd"`), to split the population into _TRUE_ vs. _FALSE_ (`cond.true` vs. `cond.false`) cases; 
2. by decision (`by = "dc"`), to split the population into _negative_ vs. _positive_ (`dec.neg` vs. `dec.pos`) decisions; 
3. by accuracy (`by = "ac"`), to split the population into _correct_ vs. _incorrect_ (`dec.cor` vs. `dec.err`) decisions. 

For instance, the following command plots a frequency tree by decisions:

```{r ex1_tree, eval = FALSE, fig.width = 7.2, fig.height = 4.2}
...
```{r ex1_area, eval = FALSE, fig.width = 7.2, fig.height = 5}
...
```{r ex1_tab, eval = FALSE, fig.width = 6, fig.height = 5}
...
```{r ex1_bar, eval = FALSE, fig.width = 7.2, fig.height = 5}
...
```{r ex1_plane, eval = FALSE, fig.width = 6, fig.height = 4.5}
...
Our selected scenario object `s21` is a list with `r length(s21)` elements, which describe it in both text and numeric variables. The following commands provide an overview of `s21` in text form:
...
```{r ex2_tree, eval = FALSE, fig.width = 7.2, fig.height = 5}
...
```{r ex2_prism_hr, eval = FALSE, fig.width = 7.2, fig.height = 5}
