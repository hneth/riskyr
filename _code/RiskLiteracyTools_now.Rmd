---
title: "Towards a Risk Literacy Toolbox"
...
Note that this file serves as an original test bed for the `riskyr` package and the corresponding Shiny app. 
(The latest code versions will be stored there, _not_ here.)
...
Define some text labels:

```{r text_settings, eval = TRUE, echo = FALSE}
...
Define graphical settings (e.g., colors and `ggplot2` themes): 
...
## Toggle between percentages and probabilities:
...
```{r}
H.lbl <- "H" # Hypothesis
E.lbl <- "E" # Evidence (E) or Decision (D)
```

_Bayes' rule_ describes the probability $p$ of a hypothesis $`r H.lbl`$ given some evidence $`r E.lbl`$ as: 

$$ p(`r H.lbl`|`r E.lbl`) = \frac{p(`r E.lbl`|`r H.lbl`) \times\ p(`r H.lbl`)} 
{p(`r E.lbl`)} $$

By further decomposing the denominator $p(`r E.lbl`)$ into two mutually exclusive parts 
$p(`r E.lbl`|`r H.lbl`) \times\ p(`r H.lbl`) + 
 p(`r E.lbl`|\neg `r H.lbl`) \times\ p(\neg `r H.lbl`)$ 
we get:
...
Information (e.g., clinical variables) is often expressed as _probabilities_ (e.g., `prev`) or even as _conditional probabilities_ (`sens`/`spec`, `ppv`/`npv`).

Many quantities can be expressed either as _probabilities_ or as _natural frequencies_. Although both representations are mathematically equivalent, a basic claim (e.g., by Gigerenzer et al.) is that people struggle when information is expressed in terms of probabilities, but not when the same information is expressed in terms of natural frequencies.

The notion of "Bayesian inference" looks and sounds complicated, but --- at its core --- we typically aim to understand the interplay between a small set of variables for a specific scenario.

### Scenario

### Scenario

Any example problem starts with the definition of a concrete _scenario_:

1. A specific _population_ of size `N`;
2. A _condition_ that is being considered 
   (with 2 states `TRUE` vs. `FALSE`);
3. A _decision_ (clinical judgment) that classifies the condition 
   (into positive or negative, or a _judgment_ of `TRUE` vs. `FALSE`);
4. Some _parameters_ (typically _probabilities_) that are _provided_ (see below);
5. Some _parameters_ (typically _probabilities_) that are _asked_ (see below).

### Environment parameters

Any specific scenario contains several numeric parameters that are either inputs or outputs.
...
4. _False alarm rate_:  
   `fart`

#### Outputs 

5. _Positive predictive value_ `ppv` 

6. _Negative predictive value_ `npv` 


#### Notes

- All these parameters are _probabilities_ 
  (on a scale from 0 to 1 or as percentages from 0% to 100%).
- Parameters 2. to 5. are defined as _conditional probabilities_.


### Number of cases

Note that we immediately translate _probabilities_ into _natural frequencies_: 

#### Truth conditions

1. Actual number of _true cases_:  

`n.true` = `N` $\times$ `prev`

2. Actual number of _false cases_: 
...
1. Number of _positive decisions_: 

`n.pos` = ...

2. Number of _negative decisions_:
...
1. Number of _hits_ (aka. _true positives_):  

`n.hi` = `n.true`            $\times$ `sens`    
       = `N` $\times$ `prev` $\times$ `sens`   
       = `n.true` - `n.mi`  

2. Number of _misses_ (aka. _false negatives_ or _omissions_) `n.mi`:

`n.mi` = `n.true` $\times$ (1 -- `sens`)   
       = (`N` $\times$ `prev`) $\times$ (1 -- `sens`)  
       = `n.true` -- `n.hi`   

3. Number of _false alarms_ (aka. _false positives_): 

`n.fa` = `n.false`                  $\times$ `fart`    
       = `N` $\times$ (1 -- `prev`) $\times$ `fart`   
       = `N` $\times$ (1 -- `prev`) $\times$ (1 -- `spec`)     
       = `n.false` -- `n.cr`   

4. Number of _correct rejections_ (aka. _true negatives_): 

`n.cr` = `n.false`                  $\times$ `spec`   
       = `N` $\times$ (1 -- `prev`) $\times$ `spec`  
       = `n.false` -- `n.fa`   
...
**Definition:** In statistics and diagnostic tests, 
the _positive predictive value_ ($PPV$) is the proportion 
of positive decisions that are true positives (hits).

In terms of natural frequencies, the $PPV$ is defined (quite simply) as: 

$$
\begin{aligned}
PPV &= \frac{\textrm{true positives}}{\textrm{positive decisions}}\\
    &= \frac{\textrm{true positives}}{\textrm{true positives} + \textrm{false positives}}\\ 
    &= \frac{\textrm{hits}}{\textrm{hits} + \textrm{false alarms}} 
\end{aligned}
$$

However, given that 

- `n.hi` = `N` $\times$ `prev` $\times$ `sens` 
- `n.fa` = `N` $\times$ (1 -- `prev`) $\times$ (1 -- `spec`) 
    
we can express this (considerably more obscurely) as a fraction of probabilities: 

$$ 
\begin{aligned}
PPV &= \frac{(N \times\ prev) \times\ sens}
            {(N \times\ prev) \times\ sens + 
             [N \times\ (1 - prev)] \times\ (1 - spec)}\\
    &= \frac{prev \times\ sens}
...
3. $PPV$ is the _reversed conditional probability_ of `sens`:

- $PPV$  = $p$(condition true | decision positive)
- `sens` = $p$(decision positive | condition true)

4. The _complement_ of $PPV$ is the _false discovery rate_ ($FDR$):
...
**Definition:** In statistics and diagnostic tests, 
the _negative predictive value_ ($NPV$) is the proportion 
of negative decisions that are true negatives (correct rejections).

1. In terms of natural frequencies, the $NPV$ is defined (quite simply) as: 

$$ 
\begin{aligned}
NPV &= \frac{true\ negatives}{negative\ decisions}\\
    &= \frac{true\ negatives}{true\ negatives + false\ negatives}\\ 
    &= \frac{correct\ rejections}{correct\ rejections\ + misses} 
\end{aligned}
$$
       
2. However, given that

- `n.cr` = `N` $\times$ (1 -- `prev`) $\times$ `spec`
- `n.mi` = `N` $\times$ `prev` $\times$ (1 -- `sens`)

we can express this (considerably more obscurely) as a fraction of probabilities:  

$$ 
\begin{aligned}
NPV &= \frac{N \times\ (1 - prev) \times\ spec}
            {(N \times\ (1 - prev) \times\ spec) + 
             [N \times\ prev \times\ (1 - sens)]} \\
    &= \frac{(1 - prev) \times\ spec}
            {(1 - prev) \times\ spec + prev \times\ (1 - sens)}
\end{aligned}
$$

3. $NPV$ is the _reversed conditional probability_ of `spec`:

- $NPV$  = $p$(condition false | decision negative)
- `spec` = $p$(decision negative | condition false)

4. The _complement_ of the $NPV$ is the _false omission rate_ ($FOR$):
...
Our approach: From theories to tools (rather than the reverse, Gigerenzer, 1991a). 

### Levels and perspectives

3 different levels with alternative perspectives:

1. Viewing population as collection of individuals or cases: table vs. icon array

2. Aggregation and categorization (SDT): confusion table, mosaic plot, tree of natural frequencies

3. Resulting posteriors: PV curves and planes


### Interactions: Sorting, sliding, and sampling: 

1. _Sorting_ is the opposite of _shuffling_ (randomization):

2. _Sliders_ and observing corresponding interactive and dynamic shifts of outputs as a function of own actions encourages viewing inputs as continuous variables (Gigerenzer et al., 1988) and thus may help to make cognitive illusions disappear (Gigerenzer, 1991b). 

3. _Sampling_ encourages differentiation of _population_ vs. _sample_ and a perspective _by description_ and one _by experience_. Allows developing and testing intuitions about sample size (Sedlmeier & Gigerenzer, 1997).
...
4. Include info on screening biases (lead time vs. over-diagnosis bias).
...
- Accuracy: acc = `r as_pc(acc)`%, bacc = `r as_pc(bacc)`%,

- Test validities: PPV = `r as_pc(ppv)`%, NPV = `r as_pc(npv)`%.
...
```{r nf-tree-diagram, include = TRUE, fig.width = 10, fig.height = 8}
...
## Function to plot tree of natural frequencies 
## (using current environment variables (NOT population) as argument:
...
# (a) choose some population: 
N <- 10^6 # (arbitrary, drops out later)
# (b) 1st row:
n.true <- (prev.range * N)
n.false <- (N - n.true)
# (c) 2nd row:
...
df <- data.frame(prev.range, PPV, NPV)
# head(df)
...
df.long$metric <- factor(df.long$metric, # ensure factor and level order:  
                         levels = c("PPV", "NPV") #,
...
# factor(df.long$metric)
  ...
  col.prev <- col.grey.2
  sens.spec.label <- paste0("(sens = ", as_pc(sens), "%, spec = ", as_pc(spec), "%)") # label
  x.just <- -.20
  y.just <- +.50
...
## A: Plot on linear scale: 
p.lin <- ggplot(data = df.long, aes(x = prev.range, y = value, group = metric)) +
...
## B: Plot on log scale:
p.log <- ggplot(data = df.long, aes(x = prev.range, y = value, group = metric)) +
  geom_line(aes(color = metric), size = 1.2) +
  geom_point(aes(color = metric, shape = metric), size = 2) +
  ...
  scale_x_log10(breaks = prev.scale) +
  # scale_x_continuous(breaks = seq(0, 1, by = .10)) +
  labs(title = paste0("PPV and NPV by prev ", sens.spec.label), x = "Prevalence (log scale)", y = "Probability") +
...
#### Plot PV curves using base R:

Generic functions for PPV and NPV:
...
## (2) Negative predictive value (NPV) from probabilities:
...
plot(0, xlim = c(0, 1), ylim = c(0, 1), type = "n", 
     main = "PV curves", ylab = "probability", xlab = "prevalence")
# abline(h = 0) # x-axis
# abline(v = 0) # y-axis

axis(side = 1, at = seq(0, 1, by = .1), col.axis = grey(.33, .6), col.ticks = grey(.66, .6))
axis(side = 2, at = seq(0, 1, by = .1), col.axis = grey(.33, .6), col.ticks = grey(.66, .6))

grid(nx = 10, ny = 10, col = grey(.8, .8))
...
## Lines:
abline(v = prev, lty = 2, lwd = 1, col = grey(.25, .50)) # show prevalence
# abline(h = PPV, lty = 3, lwd = 1, col = col.ppv) # show PPV
# abline(h = NPV, lty = 3, lwd = 1, col = col.npv) # show NPV

segments(x0 = 0, y0 = PPV, x1 = prev + eps, y1 = PPV, lty = 2, lwd = 1.5, col = col.ppv) # show PPV segment
segments(x0 = 0, y0 = NPV, x1 = prev + eps, y1 = NPV, lty = 2, lwd = 1.5, col = col.npv) # show NPV segment

## Points:
## PPV: 
points(x = prev, y = PPV, pch = 21, cex = 2, lwd = 1.5, col = grey(.25, .50), bg = col.ppv)
## NPV:
points(x = prev, y = NPV, pch = 21, cex = 2, lwd = 1.5, col = grey(.25, .50), bg = col.npv)

## Text:
## PPV label:
if ((PPV < .75 & !(prev > 1 - h.shift)) | (prev < h.shift)) {
  text(x = prev + h.shift, y = PPV + .000, labels = PPV.lbl, col = col.ppv, cex = .8) # on right
} else {
  text(x = prev - h.shift, y = PPV + .025, labels = PPV.lbl, col = col.ppv, cex = .8) # on left+
}
## NPV label: 
if (NPV > .75 | (prev < h.shift)) {
  text(x = prev + h.shift, y = NPV + v.shift, labels = NPV.lbl, col = col.npv, cex = .8) # on right+
} else {
  text(x = prev - h.shift, y = NPV - v.shift, labels = NPV.lbl, col = col.npv, cex = .8) # on left-
}
...
## (1) Define parameters:
{
  ...
  # prev <- .15 # prevalence (to be set in parameter)
...
p.ppv <- persp(x, y, z.ppv, 
               theta = my.theta, phi = my.phi,  d = my.d, expand = my.expand, 
               col = col.ppv, border = NA, 
               ltheta = 200, shade = 0.10, 
               ticktype = "detailed", 
               xlab = "sens", ylab = "spec", zlab = "PPV", zlim = z.lim, 
               main = paste0("PPV (prev = ", as_pc(prev), "%)"))

# Add cur.PPV to plot:
pmat <- p.ppv
mypoints <- trans3d(sens, spec, cur.PPV, pmat)
points(mypoints, pch = 21, col = "grey95", bg = col.ppv, cex = 1.5)

## 3D plot for NPV:
p.npv <- persp(x, y, z.npv, 
               theta = my.theta, phi = my.phi,  d = my.d, expand = my.expand, 
               col = col.npv, border = NA, 
               ltheta = 200, shade = 0.10, 
               ticktype = "detailed", 
               xlab = "sens", ylab = "spec", zlab = "NPV", zlim = z.lim, 
               main = paste0("NPV (prev = ", as_pc(prev), "%)"))
...
## (3) As a function:
## (a) PPV only:
plot.PV.plane <- function(prev, metric = "PPV") {
  
  PV.mat <- pv.matrix(prev, sens.range, spec.range, metric)
  ...
  p.pv <- persp(x, y, z.pv, 
               theta = my.theta, phi = my.phi, d = my.d, expand = my.expand, 
               col = col.ppv, border = col.grey.3,  
               ltheta = 200, shade = 0.10,
               ticktype = "detailed", xlab = "sens", ylab = "spec", 
               zlab = "PPV", 
               zlim = z.lim, 
               main = paste0("PPV (prev = ", as_pc(prev), "%)"))
...
## Check: 
plot.PV.plane(prev, metric = "PPV")

## (b) Both in 1 plot:
plot.PV.planes <- function(prev) {
...
as_pc(BxA[1,1]/sum(BxA[ , 1]))
as_pc(BxA[1,2]/sum(BxA[ , 2]))

# 1st col percentages per row (pA)
as_pc(BxA[1,1]/sum(BxA[1, ]))
as_pc(BxA[2,1]/sum(BxA[2, ]))
