...
Note that this file served as an original test bed for the `riskyr` package and the corresponding Shiny app. 
(The latest code versions are now stored there, _not_ here.)
...
```{r}
H.lbl <- "H" # Hypothesis
E.lbl <- "D" # Evidence (E) or Decision/Data (D)
```
...
## (using default objects num, freq, txt, pal):
...
Second version (for `riskyr` package):

```{r nftree-diagram_v2, include = TRUE, fig.width = 10, fig.height = 6}
## Load required packages:
library("diagram")
# library("shape")
...
plot_PV <- function(prev, sens, spec, 
