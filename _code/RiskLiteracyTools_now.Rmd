...
# Specify a vector of prevalences:
{
  ...
  prev.x <- sort(unique(c(prev.0, prev.1, prev.2, prev.3, prev.4, prev.5)))
  # prev.x
...
PPV <- get.PPV(prev.x, sens, spec)
NPV <- get.NPV(prev.x, sens, spec)
...
p.lin <- ggplot(data = df.long, aes(x = prev.x, y = value, group = metric)) +
  geom_line(aes(color = metric), size = 1.2) +
  geom_point(aes(color = metric, shape = metric), size = 2) +
  geom_line(aes(x = prev), color = "firebrick", linetype = 3, size = 1) +
...
## B: Plot on log scale:
p.log <- ggplot(data = df.long, aes(x = prev.x, y = value, group = metric)) +
  geom_line(aes(color = metric), size = 1.2) +
  geom_point(aes(color = metric, shape = metric), size = 2) +
  geom_line(aes(x = prev), color = "firebrick", linetype = 3, size = 1) +
  #geom_smooth(aes(color = metric), size = 1.2) +
