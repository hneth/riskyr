...
## PPV label:
if (PPV < .75) {
  text(x = prev + .12, y = PPV + .000, labels = PPV.lbl, col = col.ppv, cex = .8)
} else {
  text(x = prev - .12, y = PPV + .025, labels = PPV.lbl, col = col.ppv, cex = .8)
}
## NPV label: 
if (NPV > .75) {
  text(x = prev + .12, y = NPV + .025, labels = NPV.lbl, col = col.npv, cex = .8)
} else {
  text(x = prev - .12, y = NPV - .025, labels = NPV.lbl, col = col.npv, cex = .8)
}
...
legend("bottom",
       legend = c("PPV", "NPV"), 
       col = c(col.ppv, col.npv), lty = 1, lwd = 2, cex = 1, bty = "n")
