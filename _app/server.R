...
shinyServer(function(input, output, session){
  ...
  observeEvent({
    ...
    ## Define corresponding vector of true states (conditions):
    data$truth <- c(rep(TRUE, data$n.true), rep(FALSE, data$n.false))

    ## (2) Determine N of decisions:
    ...
    ## Define corresponding vector of decisions (ordered by truth values):
    data$decision <- c(rep(TRUE, data$n.hi), rep(FALSE, data$n.mi),
                       rep(TRUE, data$n.fa), rep(FALSE, data$n.cr))

    ## (3) Vector of SDT (status decision/truth):
