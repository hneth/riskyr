...
<!-- Introduction: Frequencies and Probabilities --> 
...
|  8.  | `hi` | The number of hits (true positives: condition `TRUE` & decision `TRUE`). |
|  9.  | `mi` | The number of misses (false negatives: condition `TRUE` & decision `FALSE`). |
|  10. | `fa` | The number of false alarms (false positives: condition `FALSE` & decision `TRUE`). |
|  11. | `cr` | The number of correct rejections (true negatives: condition `FALSE` & decision `FALSE`). |
...
- From four essential subsets to various combinations of them (_bottom-up_):<br>
As the four frequencies `hi`, `mi`, `fa`, and `cr` are not further split into subgroups, we can think of them as atomic elements or _essential_ frequencies. All other frequencies in `freq` are sums of various combinations of these four essential frequencies. This implies that the entire network of frequencies and probabilities (shown in the network diagram above) can be reconstructed from these four essential frequencies.
