...
Fortunately, the matrix and its implications are quite simple and straightforward. In the following, we try to disentangle the profusion of measures and summarize the parts of the confusion matrix that a risk-literate person really needs to know.
...
Goal: Measure some particular aspect of prediction performance.

Question: How good is our decision or diagnostic test in correctly detecting `cond.true` and `cond.false` cases?

As we are dealing with two types of correct decisions (`hi` and `cr`) and two perspectives (by columns vs. by rows), we can provide 4 answers to this question.

To provide a numeric quantity, we divide the frequency of correct cases (either `hi` or `cr`) by

(a) column sums (`cond.true` or `cond.false`): This yields the decision's sensitivity (`sens`) and specificity (`spec`):
...
(b) row sums (`dec.pos` or `dec.neg`): This yields the decision's positive or negative predictive value (`PPV` or `NPV`): 
...
### B. General metrics: Overall accuracy

Goal: Summarize overall performance (i.e., entire matrix) in one quantity.

`riskyr` currently uses three general metrics (which are contained in `accu`):
...
> Weighted accuracy `wacc` is the average of sensitivity (`sens`) weighted by `w`, and specificity (`spec`), weighted by `1 - w`. 
