...
```{r prism_plot_1, message = FALSE, fig.width = 7.2, fig.height = 6, fig.show = 'asis', fig.cap = "A network diagram showing frequencies as nodes and probabilities as edges linking nodes."}
...
The following 11 probabilities are distinguished by `riskyr` and contained in `prob`:
...
2. The conditional probabilities (2.--5. and 7.--10.) can be computed as a proportion of the reference group on which they are conditional. More specifically, if we schematically read each definition as "The conditional probability of $X$ provided that $Y$", then the ratio of the corresponding frequencies is `X & Y`/`Y`. More explicitly, 
...
```{r prism_plot_2, fig.width = 7.2, fig.height = 6, fig.show = 'asis', fig.cap = "A simple network diagram that shows how probabilities can be computed as ratios of frequencies."}
