...
For our present purposes, the notion of risk-related information refers to any scenario in which some events are determined by probabilities. A benign example of a risk-related situation is the `riskyr` start-up message: Every time you load the package, a randomization mechanism is invoked and determines which particular out of a range of possible messages is shown. Even if you notice this, determining the exact probability of each message would require extensive experience, explicit information (e.g., us telling you that we use R's `sample` function to randomly select 1 out of 5 possible messages), or cheating (by looking at the source code). In real life, both our amount of experience and options for cheating are more limited, unfortunately, which is why we need to rely on science for figuring out probabilities and on the transparent risk communication to understand them.

### Promoting risk perception and communication

`riskyr` facilitates the risk perception and promotes a deeper understanding of risk-related information in three ways:^[The `riskyr` logo (showing a dice's different facets) also represents its functionality in a variety of ways:  
