...
plot_tab <- function(prev = num$prev,    # probabilities
  ...
  if (by_top == "cd") {
    ...
    cond.true_x  <- c1_x  # prev/2
    cond.false_x <- c2_x  # prev + (1 - prev)/2 + v_gap

    if (brd_w > 0) {

      # Define 2 horizontal boxes (with optional scale_x):
      box_cond.true  <- make_box("cond.true",  (cond.true_x * scale_x),  tbar_y, (sdt_lx * scale_x), bar_lx)  # cond.true
      box_cond.false <- make_box("cond.false", (cond.false_x * scale_x), tbar_y, (sdt_lx * scale_x), bar_lx)  # cond.false
    ...
    dec.pos_x <- c1_x  # ppod/2
    dec.neg_x <- c2_x  # ppod + (1 - ppod)/2 + v_gap

    if (brd_w > 0) {

      # Define 2 horizontal boxes (with optional scale_x):
      box_dec.pos <- make_box("dec.pos", (dec.pos_x * scale_x), tbar_y, (sdt_lx * scale_x), bar_lx)  # dec.pos
      box_dec.neg <- make_box("dec.neg", (dec.neg_x * scale_x), tbar_y, (sdt_lx * scale_x), bar_lx)  # dec.neg
    ...
    dec.cor_x <- c1_x  # acc/2
    dec.err_x <- c2_x  # acc + (1 - acc)/2 + v_gap

    if (brd_w > 0) {

      # Define 2 horizontal boxes (with optional scale_x):
      box_dec.cor <- make_box("dec.cor", (dec.cor_x * scale_x), tbar_y, (sdt_lx * scale_x), bar_lx)  # dec.cor
      box_dec.err <- make_box("dec.err", (dec.err_x * scale_x), tbar_y, (sdt_lx * scale_x), bar_lx)  # dec.err
    ...
    warning("Plotting bottom fboxes (row 3): Unknown by_top argument/perspective!")
  ...
  if (by_bot == "cd") {
    ...
    cond.false_y  <- r2_y  # (1 - prev)/2
    cond.true_y   <- r1_y  # (1 - prev) + prev/2 + h_gap

    if (brd_w > 0) {

      # Define 2 vertical boxes (withOUT scale_x):
      box_cond.true  <- make_box("cond.true",  (lbar_x * scale_x), cond.true_y,  (bar_lx * scale_x), sdt_ly)  # cond.true
      box_cond.false <- make_box("cond.false", (lbar_x * scale_x), cond.false_y, (bar_lx * scale_x), sdt_ly)  # cond.false
    ...
    dec.neg_y <- r2_y  # (1 - ppod)/2
    dec.pos_y <- r1_y  # (1 - ppod) + ppod/2  + h_gap

    if (brd_w > 0) {

      # Define 2 vertical boxes (withOUT scale_x):
      box_dec.pos <- make_box("dec.pos", (lbar_x * scale_x), dec.pos_y, (bar_lx * scale_x), sdt_ly)  # dec.pos
      box_dec.neg <- make_box("dec.neg", (lbar_x * scale_x), dec.neg_y, (bar_lx * scale_x), sdt_ly)  # dec.neg
    ...
    dec.err_y <- r2_y  # (1 - acc)/2                # OR (rounded freq):
    dec.cor_y <- r1_y  # (1 - acc) + acc/2 + h_gap  # OR (rounded freq): (freq$dec.cor/freq$N * 1/2)

    if (brd_w > 0) {

      # Define 2 vertical boxes (withOUT scale_x):
      box_dec.cor <- make_box("dec.cor", (lbar_x * scale_x), dec.cor_y, (bar_lx * scale_x), sdt_ly)  # dec.cor
      box_dec.err <- make_box("dec.err", (lbar_x * scale_x), dec.err_y, (bar_lx * scale_x), sdt_ly)  # dec.err
    ...
    warning("Plotting right fboxes (col 3): Unknown by_bot argument/perspective!")
  ...
  plot_fbox_list(fbox_N, # plot only 1 box (in row 3, col 3):
                 cur_freq = freq, cur_txt = lbl_txt, cur_pal = col_pal,  # PASS current freq/txt/pal arguments!
                 lbl_type = f_lbl, cex = cex_lbl, lwd = f_lwd)  # No ...!
  ...
  if ( !is.null(p_lbl) && !is.na(p_lbl) ) {
    ...
    if (by_top == "cd") {
      ...
      if (p_split == "v") { # v01:
        ...
        # prev [horizontal r3: from cond.true (r3c1) to N (r3c3)]:
        ...
        if (by_bot == "dc") { # v01:
          ...
          # spec [vertical c2: from cr to cond.false)]:
        ...
        if (by_bot == "dc") { # v03:
          ...
          #           col.fill = "black", col.txt = "black",  # WAS: p_col_1
          #           srt = 90, lbl.pos = 3, lbl.off = 0, cex = cex_p_lbl)

          # PPV (horizontal, top r1): hi-dec.pos
          ...
          # acc (vertical, right c3): dec.cor-N
  ...
  if (by_top == "cd") {
    ...
    if (brd_w > 0) {
      ...
      plot_ftype_label("cond.true", (sdt_lx * scale_x), (n_rows + brd_w),
                       ...
                       col = pal["txt"], cex = cex_lbl)  # Condition (center, horizontal)

    } # if (brd_w > 0)

    # 2 sub-group labels:
    plot_freq_label("cond.true", (cond.true_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("cond.false", (cond.false_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
    ...
    if (brd_w > 0) {
      ...
      plot_ftype_label("dec.pos", (sdt_lx * scale_x), (n_rows + brd_w),
                       cur_txt = lbl_txt, suffix = ":", pos = NULL,
                       col = pal["txt"], cex = cex_lbl)  # Decision (center, horizontal)

    } # if (brd_w > 0)

    # 2 sub-group labels:
    plot_freq_label("dec.pos", (dec.pos_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("dec.neg", (dec.neg_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
    ...
    # 2 sub-group labels:
    plot_freq_label("dec.cor", (dec.cor_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("dec.err", (dec.err_x * scale_x), (n_rows + h_gap + h_shift), lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", pos = 3, col = pal["txt"], cex = cex_lbl)
  ...
  if (by_bot == "cd") {
    ...
    plot_freq_label("cond.true",  ((0 - v_shift) * scale_x), cond.true_y,  lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("cond.false", ((0 - v_shift) * scale_x), cond.false_y, lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
    ...
    plot_freq_label("dec.pos", ((0 - v_shift) * scale_x), dec.pos_y, lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("dec.neg", ((0 - v_shift) * scale_x), dec.neg_y, lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
    ...
    plot_freq_label("dec.cor", ((0 - v_shift) * scale_x), dec.cor_y, lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
    plot_freq_label("dec.err", ((0 - v_shift) * scale_x), dec.err_y, lbl_type = f_lbl_hd ,
                    cur_freq = freq, cur_txt = lbl_txt,
                    suffix = ":", srt = 90, pos = 3, col = pal["txt"], cex = cex_lbl)
