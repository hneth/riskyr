...
#' \code{\link{freq}}. Using the option \code{round = FALSE}
#' turns off rounding.
#'
#' Key relationships:
#'
...
comp_freq_prob <- function(prev = prob$prev,  # 3 essential probabilities (removed: mirt & fart ):
    ...
    if (is_valid_prob_set(prev = prev, sens = sens, mirt = mirt, spec = spec, fart = fart, tol = 10^-6)) {
...
#'
#' # Ways to work:
#' comp_freq_prob(prev = 1, sens = 1, spec = 1, 101)  # => ok + warning: N hits (TP)
#' comp_freq_prob(prev = 1, sens = 1, spec = 0, 102)  # => ok + warning: N hits
#' comp_freq_prob(prev = 1, sens = 0, spec = 1, 103)  # => ok + warning: N misses (FN)
#' comp_freq_prob(prev = 1, sens = 0, spec = 0, 104)  # => ok + warning: N misses
#' comp_freq_prob(prev = 0, sens = 1, spec = 1, 105)  # => ok + warning: N correct rejections (TN)
#' comp_freq_prob(prev = 0, sens = 1, spec = 0, 106)  # => ok + warning: N false alarms (FP)
#'
#' # Watch out for:
#' comp_freq_prob(prev = 1, sens = 1, spec = 1, N = NA)  # => ok + warning: N hits (TP) + warning: N = 1 computed
#' comp_freq_prob(prev = 1, sens = 1, spec = 1, N =  0)  # => ok, but all 0 + warning (extreme case: N hits, NPV = NaN)
#' comp_freq_prob(prev = .5, sens = .5, spec = .5, N = 10, round = TRUE)   # => ok, but all rounded (increasing errors: mi and fa)
#' comp_freq_prob(prev = .5, sens = .5, spec = .5, N = 10, round = FALSE)  # => ok, but not rounded
#'
...
comp_prob_prob <- function(prev = prob$prev,             # probabilities: 3 essential, 2 optional (mirt, fart)
                           sens = prob$sens, mirt = NA,  # Defaults: probability info contained in prob!
                           spec = prob$spec, fart = NA) {

  ## Wrapper function: Delegate to existing and more basic function:
  prob <- comp_prob(prev,             # probabilities:
                    sens, mirt = NA,  # 3 essential (prev, sens, spec)
                    spec, fart = NA)  # 2 optional  (      mirt, fart)
...
##   comp_prob_freq
##   TO
##   trans_prob (CHANGED INTO "comp_freq_prob" )
##   AND
