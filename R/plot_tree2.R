...
plot_tree2 <- function(prev = num$prev,             # probabilities
                       ...
                       ## Options:
                       by = "cd",     # 4 perspectives: "cd", "dc", "cddc", "dccd"
                       ...
                       ## Colors:
                       col.boxes = pal[c(1:9)],  # 9 possible frequencies/boxes/colors
  ...
  if (by == "cd") {  # by condition:
    ...
    col.boxes <- col.boxes[c(1:3, 6:9)]  # select 7 of 9 that include "by condition"

  } else if (by == "dc") {  # by decision:

    col.boxes <- col.boxes[c(1, 4:9)  ]  # select 7 of 9 that include "by decision"
  ...
  if (by == "cd") {  # by condition:
    ...
    if (area == "no") {  # default labels:
      ...
      M[2, 1] <- "ppod" # ERROR: WHY does prev.lbl not work with spaces???
      M[3, 1] <- ""     # "n(N - dec.pos)"
      M[4, 2] <- "PPV"
  ...
  if (area == "sq") {
    ...
    if (by == "cd") {  # (a) by condition:
      ...
      x.left  <- sqrt(n.true/N  * x.pop^2)  # cond.true cases
      x.right <- sqrt(n.false/N * x.pop^2)  # cond.false cases

      if (!all.equal(x.pop^2, sum(x.left^2, x.right^2))) {
        warning("sumtree 1a: Sum of cond.true and cond.false area differs from population area.")
      }

    } else if (by == "dc") {  # (b) by decision:

      x.left  <- sqrt(n.pos/N * x.pop^2)  # dec.pos cases
      x.right <- sqrt(n.neg/N * x.pop^2)  # dec.neg cases
    ...
    ## Collect all sizes and proportions:
    x.boxes <- c(x.pop,  x.left, x.right,  x.hi, x.mi, x.fa, x.cr)
  ...
  # compute PVs from current frequencies:
  cur.ppod <- n.pos/N
  cur.ppv  <- n.hi/n.pos
  cur.npv  <- n.cr/n.neg

  add.dc.lbl <- paste0("(", "ppod = ", as_pc(cur.ppod), "%, ", "PPV = ", as_pc(cur.ppv), "%, ", "NPV = ", as_pc(cur.npv), "%)")
